...
# Configure Cloudinary using settings from config.py
cloudinary.config(
...
def fmt(e: dict) -> EnrollmentOut:
    return EnrollmentOut(
        ...
        screenshot=e.get("screenshot"), # This ensures admin can see the receipt
...
async def payment_submit(
    ...
    screenshot: UploadFile = File(...), # Matches 'screenshot' in PaymentPanel.jsx
    db=Depends(get_db)
):
    try:
        ...
        result = await db.enrollments.insert_one({
            ...
            "screenshot": file_url, # URL for Admin Panel visibility
            "status": "pending",
            "admin_notes": None,
            "created_at": now,
            "updated_at": now,
        ...
        # Catching errors to prevent 500 Internal Server Error
...
async def payment_status(enrollment_id: str, db=Depends(get_db)):
    ...
    # If approved, provide the link to the student
    if status == "confirmed":
        response["whatsapp_link"] = "https://chat.whatsapp.com/IJ6voqFQc4U7y3HwR7kvjl?mode=gi_t"
