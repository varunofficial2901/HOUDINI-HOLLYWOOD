...
import {
  ...
  ConfirmModal, Toast, Select, Btn
} from "../components/UI";
import {
  GraduationCap, Search, Trash2, ChevronDown, ChevronUp,
  RefreshCw, Receipt, ExternalLink, ImageOff, Check
} from "lucide-react";
...
function EnrollRow({ e, onUpdate, onDelete }) {
  ...
  const save = async () => {
    setSaving(true);
    try {
      await onUpdate(e.id, { status, admin_notes: notes });
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="card mb-2 overflow-hidden">
      ...
      {open && (
        <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {/* LEFT — Student Details + Receipt */}
          <div className="space-y-2 text-sm">
            <div>
              <span style={{ color: "var(--muted)" }}>Phone: </span>
              <span style={{ color: "var(--text)" }}>{e.country_code} {e.phone}</span>
            </div>
            <div>
              <span style={{ color: "var(--muted)" }}>Gender: </span>
              <span style={{ color: "var(--text)" }}>{e.gender || "—"}</span>
            </div>
            <div>
              <span style={{ color: "var(--muted)" }}>Plan: </span>
              <span style={{ color: "var(--accent)", fontWeight: 700, textTransform: "uppercase" }}>{e.plan}</span>
            </div>
            <div>
              <span style={{ color: "var(--muted)" }}>Billing: </span>
              <span style={{ color: "var(--text)" }}>{e.billing}</span>
            </div>
            <div>
              <span style={{ color: "var(--muted)" }}>Submitted: </span>
              <span style={{ color: "var(--text)" }}>{new Date(e.created_at).toLocaleString()}</span>
            </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {/* APPROVE BUTTON — only shows when pending */}
              {(status === "pending" || e.status === "pending") && (
                <button
                  onClick={handleApprove}
                  disabled={approving}
                  className="flex items-center gap-1 px-4 py-2 rounded-lg text-sm font-bold transition hover:opacity-90 disabled:opacity-60"
                  style={{ background: "#16a34a", color: "white" }}
                >
                  <Check className="w-3 h-3" />
                  {approving ? "Approving..." : "Approve & Send Link"}
                </button>
              )}

              <Btn onClick={save} loading={saving} size="sm">
                Save Changes
              </Btn>
            ...
            {/* WHATSAPP LINK — appears after approval */}
            {whatsappLink && (
              <div className="mt-3 p-3 rounded-lg space-y-2"
                style={{ background: "rgba(22,163,74,0.1)", border: "1px solid rgba(22,163,74,0.3)" }}>
                <p className="text-xs font-bold" style={{ color: "#4ade80" }}>
                  ✅ Student Approved! Send this WhatsApp link:
                </p>
                <a
                  href={whatsappLink}
                  target="_blank"
                ...
                <p className="text-xs" style={{ color: "var(--muted)" }}>
                  The student's screen will automatically show this link within 5 seconds.
                </p>
              </div>
            )}
          </div>
